...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for proposal in orm.Proposal.objects.all():
            proposal_count = orm.Proposal.objects.filter(project__customer=proposal.project.customer,
                                                         id__lt=proposal.id).count()
            reference = '%s-%s-%s.%s' % (repr(proposal.project.customer).lower(),
                                         'CRT',
                                         (proposal.begin_date or proposal.update_date).strftime('%y'),
                                         str(proposal_count + 1).rjust(3, '0'))
